...
export class FMLStructureObjectRenderer {
  public static render(editor: FMLEditor, name: string): string {
    ...
    window['_fieldClick'] = (name: string, field: string): void => {
    ...
    return `
        <div>
          ...
          <h5 class="node-title m-justify-between">
            ...
            <span class="m-clickable"  style="padding-inline: 0.5rem 4px; margin-left: 7px;" onclick="_objectExpand('${o.name}')">
        ...
        ${o.fields.map(f => `
          <div class="m-justify-between" style="height: 1.5rem; border-bottom: 1px solid var(--color-borders)">
            <span>${f.name}</span>
            ${editor._fml.isFieldSelectable(f) ? `<span class="m-clickable" style="padding-inline: 0.5rem" onclick="_fieldClick('${o.name}', '${f.name}')">+</span>` : ''}
          </div>
        `).join('\n')}
    `;
  }
